...
void displaySpecialtyQueueSummary(void) {
    printf("\n--------------------------------------------------\n");
    printf("            SPECIALITY QUEUE SUMMARY               \n");
    printf("--------------------------------------------------\n");
    for (int i = 0; i < 4; i++) {
        int count = specialityQueueTracker[i];
        int estWait = count * specialityConsultantTime[i];
        printf("%d. %-22s | Queue:%d patients | Est.Wait:%d mins\n",
               i + 1, specialityTitles[i], count, estWait);
    }
    printf("--------------------------------------------------\n");
}
...
void registrationPatient(void) {
    ...
    int choiceIdx = specialtyChoice[patientCount] - 1;
    if (choiceIdx >= 0 && choiceIdx < 4) {
        specialityQueueTracker[choiceIdx]++;
    }
